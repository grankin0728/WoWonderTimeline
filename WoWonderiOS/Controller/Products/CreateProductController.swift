    ...
    private func createProduct(data : [Data]?) {
        ...
        case .online(.wwan),.online(.wiFi):
            ...
            performUIUpdatesOnMain {
                CreateProductManager.sharedInstance.createProduct(producName: self.productName.text!, price: Int(self.productPrice.text!)!, currency: self.currencyId, location: self.location.text!, categoryId: self.categoryId, description: self.productDescription.text!, type: self.productType, data: data) { (success, authError, error) in
                    if success != nil {
