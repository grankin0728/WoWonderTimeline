...
class JobDetailController: UIViewController,EditJobDelegate,EditJobDataDelegate,DeleteJobDelegate{
    ...
    private func loadData(){
        ...
        if let page = self.jobData["page"] as? [String:Any]{
            ...
            if let pageOwner = page["is_page_onwer"] as? Bool {
                if pageOwner == true {
                    self.appliesBtn.setTitle("\("SHOW APPLIES")\("(\(self.applyCount))")", for: .normal)
                    self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Edit", style: .done, target: self, action: #selector(self.EditJob(_:)))
                }
                else {
    ...
    func deleteJob(jobId: Int) {
        let alert = UIAlertController(title: "", message: "Are you sure Want to delete", preferredStyle: .actionSheet)
