        ...
        if multiImageArray.count == 2 {
            ...
            cell.addREact = {[unowned self] in
                print(self.selectedIndex)
                  let cell = self.tableView.cellForRow(at: IndexPath(row: self.selectedIndex, section: 5)) as! MultiImage2
                  cell.addREact?()
                ...
                case .online(.wwan), .online(.wiFi):
                    if let images_count = index["photo_multi"] as? [[String:Any]]{
                        if images_count.count == 2{
                            let cell = self.tableView.cellForRow(at: IndexPath(row: indexpath.row, section: 5)) as! MultiImage2
                            if let reactions = index["reaction"] as? [String:Any]{
                                var totalCount = 0
                                if let count = reactions["count"] as? Int{
                                    totalCount = count
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            if let images_count = self.postArray[gesture.view!.tag]["photo_multi"] as? [[String:Any]]{
                if images_count.count == 2{
                    let cell = self.tableView.cellForRow(at: IndexPath(row: gesture.view!.tag, section: 5)) as! MultiImage2
                    ...
                    let cell = self.tableView.cellForRow(at: IndexPath(row: gesture.view!.tag, section: 5)) as! MultiImage3
                    if let reactions = self.postArray[gesture.view!.tag]["reaction"] as? [String:Any]{
                        var totalCount = 0
                        if let count = reactions["count"] as? Int{
                            totalCount = count
...
//        if let images_count =
//            self.postArray[selectedIndex]["photo_multi"] as? [[String:Any]]{
//            if images_count.count == 2{
...
//            }
                ...
                let cell = self.tableView.cellForRow(at: IndexPath(row: self.selectedIndex, section: 5)) as! MultiImage3
                self.reactions(index: self.selectedIndex, reaction: reation)
                var localPostArray = self.postArray[self.selectedIndex]["reaction"] as! [String:Any]
                var totalCount = 0
                if let reactions = self.postArray[self.selectedIndex]["reaction"] as? [String:Any]{
