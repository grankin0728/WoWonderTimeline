...
//                cell.profileName.text! = profilename
        ...
        if let commentsCount = index["post_comments"] as? String{
            cell.commentCountBtn.setTitle("\("Comments ")\(commentsCount)", for: .normal)
        }
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            let cell = self.tableView.cellForRow(at: IndexPath(row: gesture.view!.tag, section: 5)) as! GifImageCell
    ...
    func addReaction(reation: String) {
        let cell = self.tableView.cellForRow(at: IndexPath(row: self.selectedIndex, section: 5)) as! GifImageCell
