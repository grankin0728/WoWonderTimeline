...
class GetPostGroup : AddReactionDelegate,SharePostDelegate{
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            let cell = self.tableView.cellForRow(at: IndexPath(row: gesture.view!.tag, section: 5)) as! GroupCell
    ...
    func addReaction(reation: String) {
        let cell = self.tableView.cellForRow(at: IndexPath(row: self.selectedIndex, section: 5)) as! GroupCell
