...
//                cell.eventImage.sd_setImage(with: url, placeholderImage: nil, options: [], completed: { (theImage, error, cache, url) in
...
//        imageAttachment2.bounds = CGRect(x: 0, y: imageOffsetY, width: 11.0, height: imageAttachment.image!.size.height)
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            let cell = self.tableView.cellForRow(at: IndexPath(row: gesture.view!.tag, section: 5)) as! EventCell
    ...
    func addReaction(reation: String) {
        let cell = self.tableView.cellForRow(at: IndexPath(row: self.selectedIndex, section: 5)) as! EventCell
